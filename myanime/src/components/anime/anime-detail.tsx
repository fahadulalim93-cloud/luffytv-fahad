...
export default function AnimeDetailPage({ animeId }: AnimeDetailProps) {
  ...
  const [detailLoading, setDetailLoading] = useState(true); // episodes + anilist detail
  ...
  // Phase 1: Load basic info FAST — show hero/overview immediately
  // Phase 2: Load episodes + AniList detail in background — show when ready
  useEffect(() => {
    let cancelled = false;
    async function load() {
      setLoading(true);
      ...
      try {
        // Phase 1: Just the info (fast) — show hero immediately
        ...
        if (!cancelled) setLoading(false); // Hero can show now!

        // Phase 2: Episodes + AniList detail in parallel
        const cleanId = animeId.replace(/^miruro_/, "").replace(/^mal_/, "");
        const aid = /^\d+$/.test(cleanId) ? parseInt(cleanId) : null;
        if (aid && !cancelled) setAnilistId(aid);

        const fetches: Promise<any>[] = [
          fetch(`/api/anime/episodes?id=${encodeURIComponent(animeId)}`).then(async r => r.ok ? r.json() : null),
        ];
        if (aid) {
          fetches.push(fetch(`/api/anime/anilist-detail?id=${aid}`).then(async r => r.ok ? r.json() : null));
        }

        const [epData, detailData] = await Promise.all(fetches);
        ...
        if (!cancelled) {
          ...
          // Process AniList detail
          if (detailData) {
            setAnilistCharacters(detailData.characters || []);
            setAnilistStaff(detailData.staff || []);
            if (detailData.recommendations) setAnilistRecommendations(detailData.recommendations);
            if (detailData.relations) setAnilistRelations(detailData.relations);
            if (detailData.studios) setAnilistStudios(detailData.studios);
            if (detailData.trailer) setAnilistTrailer(detailData.trailer);
            if (detailData.details) {
              setAnilistMedia(detailData.details);
              if (detailData.details.episodes) {
                setTotalEpisodes(prev => prev ?? detailData.details.episodes);
              }
            }
          }
    ...
    return (
      <div className="flex items-center justify-center min-h-[80vh]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-[3px] border-red-500/30 border-t-red-400 rounded-full animate-spin" />
    ...
    <div className="fade-in">
      ...
      {banner && (
        <div className="relative min-h-[90vh] -mt-[75px] overflow-hidden">
          <img src={banner} alt="" className="absolute inset-0 w-full h-full object-cover ken-burns" key={`banner-${animeId}`} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
            ...
            <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row items-end gap-8">
              <div className="flex-1 space-y-4">
                  ...
                  <div className="stagger-reveal stagger-3 flex flex-wrap gap-2">
                    {allGenres.slice(0, 8).map(g => (
                      <button
                        ...
                        className="px-3 py-1 text-xs font-medium bg-red-500/10 text-purple-300 rounded-full border border-red-500/20 hover:bg-red-500/20 transition-all"
                      >
                        {g}
                      </button>
                    ))}
                ...
                <div className="stagger-reveal stagger-5 flex items-center gap-3 pt-2">
                  ...
                  <button
                    ...
                    className={`pill-btn ${bookmarked ? "bg-red-500/10 text-purple-300 border border-red-500/20" : "pill-btn-ghost"}`}
                  >
              ...
              {image && (
                <div className="stagger-reveal stagger-4 hidden lg:block shrink-0">
                  <img src={image} alt={displayTitle} className="w-[240px] rounded-xl poster-3d" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                </div>
              )}
            </div>
          </div>
      ...
      {!banner && image && (
        <div className="flex flex-col sm:flex-row gap-6 mt-4">
          <div className="shrink-0 w-[180px] mx-auto sm:mx-0">
            <img src={image} alt={displayTitle} className="w-full rounded-xl shadow-2xl shadow-black/50" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
          </div>
      ...
      {/* Bottom sections loading skeleton — shows while episodes + detail load */}
      {detailLoading && (
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              {Array.from({ length: 6 }, (_, i) => (
                <div key={i} className="shrink-0 w-[80px] h-[80px] rounded-full skeleton" />
              ))}
            </div>
          </div>
          <div className="space-y-3">
      ...
      {/* AniList Voice Cast / Characters — only show after detail loads */}
      {!detailLoading && anilistCharacters.length > 0 && (
              ...
              return (
                <div key={c.id} className="shrink-0 text-center w-[120px]">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-[80px] h-[80px] rounded-full overflow-hidden border-2 border-red-500/20 bg-[#0f0f1a] mb-1">
                      {c.image?.large || c.image?.medium ? (
                        <>
                          <img src={c.image.large || c.image.medium} alt={cName} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                          <div className="w-full h-full items-center justify-center text-sm text-zinc-600 font-semibold bg-[#0f0f1a]" style={{ display: 'none' }}>{cName.charAt(0)}</div>
                        </>
                      ) : (
                    ...
                    <span className={`text-[8px] font-bold mt-0.5 px-2 py-0.5 rounded-full ${
                      c.role === "MAIN" ? "bg-red-500/15 text-purple-300" : "bg-white/[0.05] text-zinc-400"
                    }`}>{c.role}</span>
                  </div>
                  ...
                  {va && (
                    <div className="flex flex-col items-center mt-2">
                      ...
                      <div className="w-[60px] h-[60px] rounded-full overflow-hidden border-2 border-red-500/20 bg-[#0f0f1a] mb-1">
                        {va.image?.large || va.image?.medium ? (
                          <>
                            <img src={va.image.large || va.image.medium} alt={vaName} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                            <div className="w-full h-full items-center justify-center text-xs text-zinc-600 font-semibold bg-[#0f0f1a]" style={{ display: 'none' }}>{vaName.charAt(0)}</div>
                          </>
                        ) : (
      ...
      {/* AniList Staff */}
      {!detailLoading && anilistStaff.length > 0 && (
        <div className="mt-8 space-y-3">
          <div className="section-header">
              ...
              <div key={s.id} className="shrink-0 text-center w-[100px]">
                <div className="w-[80px] h-[80px] rounded-full overflow-hidden mx-auto mb-2 border-2 border-white/[0.06] bg-[#0f0f1a]">
                  {s.image?.large || s.image?.medium ? (
                    <>
                      <img src={s.image.large || s.image.medium} alt={sName} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                      <div className="w-full h-full items-center justify-center text-sm text-zinc-600 font-semibold bg-[#0f0f1a]" style={{ display: 'none' }}>{sName.charAt(0)}</div>
                    </>
                  ) : (
      ...
      {/* AniList Studios */}
      {!detailLoading && anilistStudios.length > 0 && (
        <div className="mt-8 space-y-3">
          <div className="section-header">
          ...
          <div className="flex gap-2 flex-wrap">
            {anilistStudios.map(s => (
              <span key={s.id} className={`px-3 py-1.5 text-xs font-semibold rounded-full ${
                s.isAnimationStudio
                  ? "bg-red-500/15 text-purple-300 border border-red-500/20"
                  : "bg-white/[0.05] text-zinc-400 border border-white/[0.06]"
              }`}>
                {s.name}
                {s.isAnimationStudio && <span className="ml-1 text-[8px] text-red-500">★</span>}
              </span>
            ))}
          </div>
        </div>
      ...
      {/* AniList Trailer */}
      {!detailLoading && anilistTrailer && anilistTrailer.site === "youtube" && (
        <div className="mt-8 space-y-3">
          <div className="section-header">
      ...
      {!detailLoading && anilistRecommendations.length > 0 && (
        <div className="mt-8 space-y-3">
          <div className="section-header">
              ...
              return (
                ...
                >
                  <div className="w-[120px] aspect-[3/4] rounded-lg overflow-hidden border border-white/[0.04] bg-[#0f0f1a] mb-1.5">
                    {rImg ? (
                      <>
                        <img src={rImg} alt={rTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                        <div className="w-full h-full items-center justify-center text-xs text-zinc-600 bg-[#0f0f1a]" style={{ display: 'none' }}>{rTitle.charAt(0)}</div>
                      </>
                    ) : (
      ...
      {!detailLoading && anilistRelations.length > 0 && (
        <div className="mt-8 space-y-3">
          <div className="section-header">
            ...
            {anilistRelations.slice(0, 8).map(r => {
              ...
              return (
                <button
                  key={r.id}
                  onClick={() => navigate({ page: "anime", id: String(r.id) })}
                  className="flex items-center gap-3 w-full p-2 bg-[#131c26] rounded-lg border border-white/[0.03] hover:bg-white/[0.04] hover:border-red-500/20 transition-all group text-left"
                >
                  <div className="w-12 h-16 rounded-md overflow-hidden shrink-0 bg-[#0f0f1a]">
                    {rImg ? (
                      <>
                        <img src={rImg} alt={rTitle} className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                        <div className="w-full h-full items-center justify-center text-[8px] text-zinc-600 bg-[#0f0f1a]" style={{ display: 'none' }}>{rTitle.charAt(0)}</div>
                      </>
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 font-bold">{r.relationType?.replace(/_/g, " ")}</span>
      ...
      {!detailLoading && (episodes.length > 0 || hasMiruroEps) && (
                ...
                {(["sub", "dub"] as const).map(tab => (
                  <button
                    ...
                    className={`px-4 py-1.5 text-[11px] font-bold rounded-full transition-all ${
                      activeTab === tab ? "bg-red-500/15 text-purple-300" : "text-zinc-500 hover:text-zinc-300"
                    }`}
                  >
              ...
              ? currentEps.map((ep) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-3 p-2 bg-[#131c26] rounded-lg border border-white/[0.03] hover:bg-white/[0.04] hover:border-red-500/20 transition-all group text-left"
                    >
                      {/* Thumbnail */}
                      <div className="w-20 h-12 rounded-md overflow-hidden shrink-0 bg-[#0f0f1a] relative">
                        {epThumbnail ? (
                          <>
                            <img src={epThumbnail} alt="" className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                            <div className="absolute inset-0 flex items-center justify-center" style={{ display: 'none' }}>
                              <span className="text-xs font-bold text-red-400">{ep.number}</span>
                            </div>
                          </>
                        ) : (
                        ...
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <svg className="w-5 h-5 text-red-400" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3" /></svg>
                        </div>
                      </div>
                      {/* Info */}
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="w-20 h-12 rounded-md overflow-hidden shrink-0 bg-[#0f0f1a] relative">
                        {epThumbnail ? (
                          <>
                            <img src={epThumbnail} alt="" className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; const fb = e.currentTarget.nextElementSibling as HTMLElement; if (fb) fb.style.display = 'flex'; }} />
                            <div className="absolute inset-0 flex items-center justify-center" style={{ display: 'none' }}>
                              <span className="text-xs font-bold text-red-400">{ep.episodeIdNum}</span>
                            </div>
                          </>
                        ) : (
                          <div className="w-full h-full flex items-center justify-center">
      ...
      {!detailLoading && !episodes.length && !hasMiruroEps && episodesCount && episodesCount > 0 && (
        <div className="mt-8 space-y-4">
          <div className="section-header flex items-center gap-2">
            <h3 className="text-sm font-bold text-white">EPISODES</h3>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
            {Array.from({ length: Math.min(episodesCount, 50) }, (_, i) => i + 1).map(num => (
              <button
                ...
                className="server-pill justify-center text-[11px] font-semibold py-2.5 px-2 hover:bg-red-500/10 hover:border-red-500/20 hover:text-purple-300 transition-all"
              >
                <svg className="w-3 h-3 text-red-400/60 shrink-0" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3" /></svg>
                {num}
              </button>
            ))}
          </div>
      ...
      {!detailLoading && !hasAnyEpisodes && !loading && (
      ...
      <div className="mt-10 space-y-6">
        <div className="section-header flex items-center gap-2">
          <svg className="w-5 h-5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
        ...
        <div className="p-4 bg-[#131c26] rounded-xl border border-white/[0.04] space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="w-full sm:w-40 px-3 py-2 bg-[#05050a] border border-white/[0.06] rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-red-500/30"
            />
          ...
          <div className="flex gap-3">
            <textarea
              ...
              rows={3}
              className="flex-1 px-3 py-2 bg-[#05050a] border border-white/[0.06] rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-red-500/30 resize-none"
            />
          </div>
                  ...
                  <div className="p-4 bg-[#131c26] rounded-xl border border-white/[0.04] hover:border-white/[0.06] transition-all">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-500/20 to-red-500/20 flex items-center justify-center shrink-0 border border-white/[0.06]">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <button onClick={() => { setReplyTo(replyTo === comment.id ? null : comment.id); setReplyContent(""); }} className="text-[10px] text-zinc-500 hover:text-red-400 font-medium transition-colors">
                        Reply
                      </button>
                    </div>
                    ...
                    {replyTo === comment.id && (
                      <div className="mt-3 p-3 bg-[#05050a] rounded-lg space-y-2 border border-white/[0.03]">
                        <textarea
                          ...
                          rows={2}
                          className="w-full px-3 py-2 bg-[#0a0a14] border border-white/[0.06] rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-red-500/30 resize-none"
                        />
